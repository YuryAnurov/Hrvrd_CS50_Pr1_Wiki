...
#        lnkent = mark_safe('<a href="entries/' + ent + '.md">' + ent + '</a>') #first tried to do real path, but djungo needed one more path in urls... Still not sure what is required in task - wike/title, or just title
#        lnkent = mark_safe('<a href="' + ent + '">' + ent + '</a>') #second version - without wiki
        lnkent = mark_safe('<a href="wiki/' + ent + '">' + ent + '</a>') #finally decided to do with wiki, as all pages have links wiki/
        msp.append(lnkent)
...
#        cont = mark_safe(util.conver(name)) #this is my own converter markdown to html, before I've discovered markdown2
        pth = 'entries/' + name + '.md'
